...
public class TripServiceImplTest {
  ...
  public void setUp() {
    ...
    LongStream.range(0, BARS.size()).forEach(
        i -> Mockito.when(barDao.findById("uuid-" + (i + 1)))
            .thenReturn(Optional.of(BARS.get((int) i))));
    Mockito.when(barDao.findById("uuid-8")).thenReturn(Optional.empty());
  }

  @Test
  public void generateATripWithNegativeNumberOfBar() {
    Trip trip = mockTripService.generate(-1, Collections.emptyList());
    assertThat(trip.getBars()).isEmpty();
  }

  @Test
  public void generateATripWithNullBars() {
    Trip trip = mockTripService.generate(-1, null);
    assertThat(trip.getBars()).isEmpty();
  }

  @Test
  public void generateATripWithZeroBar() {
    Trip trip = mockTripService.generate(0, Collections.emptyList());
    assertThat(trip.getBars()).isEmpty();
  }

  @Test
  public void generateATripWithUnknownBar() {
    Trip trip = mockTripService.generate(1, ImmutableList.of("uuid-8"));
    assertThat(trip.getBars()).hasSize(1);
    assertThat(trip.getBars().get(0).getUuid()).isNotEqualTo("uuid-8");
  }

  @Test
  public void generateATripWithBarWithoutHappyHour() {
    Trip trip = mockTripService.generate(1, ImmutableList.of("uuid-7"));
    assertThat(trip.getBars()).hasSize(1);
  ...
  public void generateATripWithMoreBarsThanInDb() {
    Trip trip = mockTripService.generate(10, Collections.emptyList());
  ...
  public void generateATripWithNotEnoughBars() {
    Trip trip = mockTripService.generate(1, ImmutableList.of("uuid-1", "uuid-2"));
    assertThat(trip.getBars()).isEmpty();
  }

  @Test
  public void generateATripWithOnlyMandatoryBars() {
    Trip trip = mockTripService.generate(2, ImmutableList.of("uuid-1", "uuid-2"));
  ...
  public void generateATripWithConsecutiveHappyHours() {
    Trip trip = mockTripService.generate(3, Collections.emptyList());
  ...
  public void generateATripWithConsecutiveHappyHoursAndMandatoryBars() {
    Trip trip = mockTripService.generate(5, ImmutableList.of("uuid-1"));
